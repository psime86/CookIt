...
router.post('/user', (req, res) => {
  ...
  db.User.find({})
    ...
    .then((data) => {
      ...
      if (data == 0) {
        console.log(
          'No documents in User collection, creating document for this user now. (BACKEND)'
        );
        db.User.create({
          email: userEmail,
          name: userName,
        ...
        return db.User.find({})
          .then((data) => {
            ...
            for (let i = 0; i < data.length; i++) {
              ...
              if (data[i].uidFB == UID) {
                console.log(
                  'User found in DB. Hello: ' + userName + '! (BACKEND)'
                );
                match = true;
                ...
                .then((data) => {
                  ...
                  console.log(
                    'User: ' +
                      ...
                      userName +
                      "'" +
                      ' added to DB successfully! (BACKEND)'
                  );
                  console.log("Returning DB '_id' to front end. (BACKEND)");
                  res.json(data._id);
                })
...
router.post('/recipe', (req, res) => {
  ...
  let match = false;
  // Validate
  db.User.findById({ _id: userReference })
    .populate('recipes')
    .then((data) => {
      console.log(data);
      ...
      if (!data.recipes.length) {
        ...
        db.Recipe.create({
          ...
          return db.User.findOneAndUpdate(
            ...
            .then((data) => {
              console.log(data);
              console.log(
                "Recipe '_id' added to User 'recipes' array! (BACKEND)"
              );
              console.log(
                'attempting to return data from recipe route (BACKEND)'
              );
        ...
        for (let i = 0; i < data.recipes.length; i++) {
          ...
          if (data.recipes[i].recipeId == req.body.id) {
            ...
            console.log(
              'No match found in User recipe array, creating document and adding now (BACKEND)'
            );
          }
        }
      ...
      if (!data.shoppingList.length) {
          ...
          .then((data) => {
            ...
            return db.User.findOneAndUpdate(
              ...
              .then((data) => {
                console.log(data);
                console.log(
                  "GroceryList '_id' added to User 'shoppingList' array (BACKEND)"
                );
                console.log(
                  'attempting to return data from grocery route (BACKEND)'
                );
                return res.json(data);
              })
              .catch((err) => console.log(err));
        ...
        for (let i = 0; i < data.shoppingList.length; i++) {
          ...
          if (data.shoppingList[i].title == req.body.name) {
            match = true;
            console.log(
              'This recipe is already in your Grocery List. (BACKEND)'
            );
        ...
        if (match == false) {
          ...
          db.GroceryList.create({
            ...
            .then((data) => {
              console.log(
                'response from creating GroceryList document: ' + data
              );
              // Assign GroceryList "_id" (DB Id) to variable to use to push later on
              let dbGroceryListId = data._id;
              return db.User.findOneAndUpdate(
...
router.get('/findUser/:id', (req, res) => {
  ...
  return db.User.find({ _id: req.params.id })
    .populate('recipes')
    .populate('shoppingList')
...
router.delete('/deleteFavRecipe', (req, res) => {
  ...
  db.Recipe.findByIdAndDelete({ _id: recipeDBId })
    .then(() => {
      ...
      return db.User.findByIdAndUpdate(
        ...
        .then((data) => {
          ...
          console.log(
            'Now attempting to return data from delete route (BACKEND)'
          );
          res.json(data);
        })
        .catch((err) => console.log(err));
...
router.delete('/deleteGroceryList', (req, res) => {
  ...
  db.GroceryList.findByIdAndDelete({ _id: groceryListDBId })
    .then(() => {
      console.log(
        'GroceryList deleted from DB GroceryList collection (BACKEND)'
      );
      return db.User.findByIdAndUpdate(
        { _id: userReference },
